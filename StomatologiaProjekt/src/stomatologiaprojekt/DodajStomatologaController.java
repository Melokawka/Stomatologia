...
public class DodajStomatologaController implements Initializable {
    ...
    private void DodajButtonAction(ActionEvent event) throws ClassNotFoundException, SQLException, InstantiationException, IllegalAccessException {
        ...
        if (ImieTekst.getText().equals("") || NazwiskoTekst.getText().equals("") || RZatrudnieniaTekst.getText().equals("") || TelefonTekst.getText().equals("")
            ...
            try {
                handler = DatabaseHandler.getInstance();
                sql = "{call stomatolog_pack.dodaj(?,?,?,?,?,?,?)}";
                //Dodanie nowego rekordu
                handler.dodajStomatologa(sql, ImieTekst.getText(), NazwiskoTekst.getText(),
                        RZatrudnieniaTekst.getText(), Date.valueOf(DataUrPicker.getValue()),
                        TelefonTekst.getText(), KlinikaCombo.getSelectionModel().getSelectedItem().toString(),
                        GodzinyCombo.getSelectionModel().getSelectedIndex() == 0 ? "R" : "W");
