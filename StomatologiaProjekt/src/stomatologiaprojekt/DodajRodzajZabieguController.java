...
public class DodajRodzajZabieguController implements Initializable {
    ...
    @FXML
    private void DodajButtonAction(ActionEvent event) throws SQLException, ClassNotFoundException, InstantiationException, IllegalAccessException {
        if (NazwaText.getText().equals("") || CenaText.getText().equals("")) {
            ...
            try {
                ...
                sql = "{call rodzzab_pack.dodaj(?,?,?,?)}";
                handler.dodajRodzajZabiegu(sql, NazwaText.getText(), parseInt(CenaText.getText()), (RefundacjaText.getSelectionModel().getSelectedIndex() == 0 ? "1" : "0"), parseInt(KrotnoscText.getText()));
